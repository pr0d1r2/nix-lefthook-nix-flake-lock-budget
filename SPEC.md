...
| B13 | 2026-09-23 | The lock-graph guard found duplicated `nixpkgs`/`nixpkgs-lock` and `set-and-setting` nodes caused by missing follows on direct and nested inputs. | Added the required follows wiring and regenerated `flake.lock`; the full 26-check flake suite now passes. |
| B14 | 2026-09-23 | CI guardrails could not resolve `flake:nix-cavekit`: the dependency existed only as a nested input of `nix-dev-shell-agentic`, while the shared guard invokes it as a standalone flake; promoting it also initially duplicated `set-and-setting` in the lock graph. | Declared `nix-cavekit` as a direct input, wired its nested consumers and `set-and-setting` to follow existing inputs, and regenerated `flake.lock`. |
| B15 | 2026-09-23 | The guardrails workflow rejected the flake metadata's generated `CHANGEME` description even though the normal flake checks passed. | Replaced the placeholder with the project's actual description. |
