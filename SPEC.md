...
| id | date | cause | fix |
|----|------|-------|-----|
| B1 | 2026-07-14 | CI exit 127: `lefthook.yml` ran an absent markdownlint-agentic binary; README also had MD014 violations. | Added the missing flake input/package to `ciPackages` and removed `$` prefixes from README prompts. |
...
| B17 | 2026-09-25 | Bats tests assumed the packaged command was on dev-shell `PATH`, but the shared shell exposes only fragment tools. | Added a helper invoking the repository script directly, preserving assertions without shell wiring. |
